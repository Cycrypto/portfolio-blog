...
describe('CommentsService', () => {
  ...
  it('stores a bcrypt hash instead of the raw password when creating a comment', async () => {
    ...
    expect(commentRepository.count).toHaveBeenCalledWith({
      where: { postId: 1, isDeleted: false },
    });
    expect(postRepository.update).toHaveBeenCalledWith(1, { comments: 1 });
    expect(created.authorEmail).not.toBe('secret-1234');
    await expect(compare('secret-1234', created.authorEmail)).resolves.toBe(
      true,
    );
  });
  ...
  it('updates a legacy email-based comment and upgrades the stored credential to a hash', async () => {
    ...
    commentRepository.createQueryBuilder!.mockReturnValue(
      createQueryBuilderResult(legacyComment),
    );
    ...
    await expect(
      compare('legacy@example.com', updated.authorEmail),
    ).resolves.toBe(true);
  });
  ...
  it('treats legacy credentials starting with $2 as plain text unless they match a full bcrypt hash', async () => {
    ...
    await expect(
      compare('$2legacy@example.com', updated.authorEmail),
    ).resolves.toBe(true);
  });
  it('rejects updates when the supplied password does not match', async () => {
    ...
    commentRepository.createQueryBuilder!.mockReturnValue(
      createQueryBuilderResult(securedComment),
    );
  ...
  it('synchronizes the post comment count after deleting a comment tree', async () => {
    const rootComment = {
      id: 'comment-root',
      postId: 1,
      content: 'root',
      authorName: '작성자',
      authorEmail: 'password',
      isDeleted: false,
    } as Comment;

    commentRepository.createQueryBuilder!.mockReturnValue(
      createQueryBuilderResult(rootComment),
    );
    ...
    await service.deleteComment(1, 'comment-root', { password: 'password' });

    expect(commentRepository.update).toHaveBeenNthCalledWith(
      1,
      'comment-root',
      { isDeleted: true },
    );
    ...
    expect(postRepository.update).toHaveBeenCalledWith(1, { comments: 0 });
  });
});
