...
export class PostsController {
  ...
  @ApiOperation({
    summary: '포스트 목록 조회',
    description:
      '키워드, 태그, 페이지네이션을 이용해 포스트 목록을 조회합니다.',
  })
  ...
  async getPosts(
    ...
    const { pageNum, pageSizeNum } = this.parsePagination(page, pageSize);
  ...
  @Roles('admin')
  @ApiOperation({ summary: '관리자 포스트 목록 조회 (Admin Only)' })
  @ApiQuery({ name: 'keyword', required: false, description: '검색할 키워드' })
  @ApiQuery({ name: 'tag', required: false, description: '필터링할 태그' })
  @ApiQuery({
    name: 'page',
  ...
  async getAdminPosts(
    ...
    const [posts, totalCount] = await Promise.all([
      this.postsService.getAdminPosts(
        keyword,
        effectiveTag,
        pageNum,
        pageSizeNum,
      ),
      this.postsService.getPostsCount(keyword, effectiveTag, true),
    ]);
  ...
  @Roles('admin')
  @ApiOperation({
    summary: '새 포스트 생성 (Admin Only)',
    description: '새로운 포스트를 생성합니다. 관리자 권한이 필요합니다.',
  })
  ...
  @Roles('admin')
  @ApiOperation({
    summary: '포스트 수정 (Admin Only)',
    description: '기존 포스트를 수정합니다. 관리자 권한이 필요합니다.',
  })
  ...
  @ApiResponse({
    ...
    type: PostEditResponseDTO,
  })
  async convertPostToTiptap(
    @Param('id') id: string,
    @Body() updatePostDTO: UpdatePostRequestDTO = {},
  ): Promise<PostEditResponseDTO> {
    const post = await this.postsService.convertMarkdownPostToTiptap(
      parseInt(id),
      updatePostDTO,
    );
    if (!post) {
  ...
  @Roles('admin')
  @ApiOperation({
    summary: '포스트 삭제 (Admin Only)',
    description: '포스트를 삭제합니다. 관리자 권한이 필요합니다.',
  })
  ...
  private toPostEditResponseDTO(post: Post): PostEditResponseDTO {
    ...
    postEditResponseDTO.tags = post.tags
